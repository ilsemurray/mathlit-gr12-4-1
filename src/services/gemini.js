/**
 * gemini.js — Service for all Google Gemini API calls
 * Replaces claude.js. All AI-powered features route through this file.
 *
 * Setup: Add VITE_GEMINI_API_KEY to your Vercel environment variables.
...
async function callGemini(systemPrompt, userPrompt, questionBankText = "") {
  ...
  const questionBankSection = questionBankText
    ? `\n\nQUESTION BANK REFERENCE (use these as style and difficulty reference only — do not copy questions verbatim):\n${questionBankText.slice(0, 8000)}`
    : "";
...
// ─────────────────────────────────────────────────────────────────
export async function generateTopicAssessment(topic, teacher, questionBankText = "") {
...
Separate the assessment and memorandum clearly with "--- MEMORANDUM ---"`;
  ...
  return callGemini(system, user, questionBankText);
}
...
export async function generateFormalAssessment({
  ...
  const topicList = selectedTopics.map((t) => `- ${t.topic}: ${t.subtopic}`).join("\n");
...
TOPICS TO ASSESS (set questions ONLY on these topics):
${topicList}
...
Topics to cover in this investigation:
${topicList}
